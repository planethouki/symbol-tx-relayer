...
test('cosignature', async () => {
    ...
    sign
        ...
        .concatCosignature();
    expect(sign.cosignedTransaction).toBeInstanceOf(SignedTransaction);
});

test('parentHash', async () => {
    const sign = new Sign({
        signature,
        publicKey,
        parentHash
    });
    expect(sign.parentHash).toBe(parentHash);
});
...
test('send', async () => {
    ...
    await Sign
        ...
        .then((res) => {
            console.log(res);
            expect(res).toBeInstanceOf(TransactionAnnounceResponse)
        });
});
